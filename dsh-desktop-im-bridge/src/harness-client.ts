...
export class HarnessClient implements HarnessBridge {
    ...
    ws.addEventListener('message', (ev: { data: unknown }) => {
      void (async () => {
        try {
          ...
          // The agent asked the human a question. method = frame type, so a
          // question arrives as its own server-request (not a session/event).
          if (frame.method === 'question/requested') {
            if (payload.sessionId !== sessionId) return
            const request: ImQuestionRequest = {
              rpcId: frame.rpcId,
              sessionId: payload.sessionId,
              questions: Array.isArray(payload.questions) ? payload.questions : [],
            }
            // Await the handler: the turn stays blocked until the answer (or a
            // fallback) is submitted through answerQuestion(). Never reject —
            // a throw here would leave the agent blocked forever.
            try {
              await handlers.onQuestion?.(request)
            } catch {
  ...
  async answerQuestion(
    ...
    try {
      ...
      if (data.accepted === true) return { accepted: true }
      return data.reason ? { accepted: false, reason: data.reason } : { accepted: false }
    } catch (cause) {
      return { accepted: false, reason: cause instanceof Error ? cause.message : String(cause) }
    }
  }

  /** Cancel a pending question (aborts the ask_user_question tool call). */
  async cancelQuestion(request: ImQuestionRequest): Promise<void> {
    try {
      await fetch(`${this.base}/api/respond`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          type: 'client-response',
          rpcId: request.rpcId,
          result: { ok: false, error: { code: 'cancelled', message: 'cancelled by im-gateway' } },
        }),
      })
    } catch {
      /* ignore */
