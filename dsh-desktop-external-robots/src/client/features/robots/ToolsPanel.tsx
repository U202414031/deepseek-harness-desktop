...
/**
 * Desktop-owned "外部机器人 / IM 网关" panel (left column).
 *
...
const CHANNEL_META: Record<ChannelType, ChannelMeta> = {
  ...
  feishu: {
    ...
    note: '飞书自建应用：开通「机器人」能力，并在「事件订阅 → 长连接」订阅 im:message.receive_v1 事件（无需公网回调地址）。填好 AppID/AppSecret 并启用后，本机自动建立长连接，在飞书里单聊机器人即可与 DeepSeek 对话。',
  },
  weixin: {
    label: '微信',
    emoji: '💬',
    fields: [
      ...
      { key: 'token', label: '回调 Token', placeholder: '企业微信后台「接收消息」Token' },
      { key: 'encodingAESKey', label: '回调 EncodingAESKey', secret: true, placeholder: '企业微信后台「接收消息」EncodingAESKey（43 位）' },
      { key: 'callbackPort', label: '回调端口（默认 9909）', placeholder: '9909' },
    ],
    docUrl: 'https://developer.work.weixin.qq.com/document/path/90930',
    note: '个人微信无官方机器人接口，此处接企业微信自建应用。出站（给成员发消息）只需 CorpID/Secret/AgentId；入站需配置「接收消息」回调：用 cpolar/ngrok 等把本机回调端口映射为公网 HTTPS 地址（如 https://xxx.cpolar.cn/wecom/callback），填入企业微信后台，并把下方 Token/EncodingAESKey 填好。手机企业微信里给应用发消息即可与 DeepSeek 对话。',
  },
}
  ...
  return (
    <div className="dshDesktopTools">
      <header className="dshDesktopFeatureHeader">
        <h2 className="dshDesktopFeatureTitle">外部机器人 · IM 网关</h2>
